...
fcolsum = zeros(1,1000);

fcolsum = sum(fbl);

% Calculating probability and entropy values

for i =1:600

    for j=1:1000
       pfbl(i,j) = fbl(i,j)/fcolsum(j); 
       entfft(i,j) = (-(pfbl(i,j)*log2(pfbl(i,j)))-((1-pfbl(i,j))*log(1-pfbl(i,j))));
    end
end

% calculating feature entropies 
encols = sum(entfft);
% finding indices oof top 120 features

[V,I] = sort(encols);

% storing top 120 indices
for i=1:120
topind(i) = I(i);
end

% Retrieving top 120 fft components for all songs

for i=1:600
   for j=1:120
      finfft(i,j) = fbl(i,topind(j)); 
   end
end
...
for i=1:600
    ...
    for j=1:120
        ...
        %finfft(i,j) = fbl(i,j)/sum(fbl(:,j)); 
        finfft(i,j) = finfft(i,j)/M(j);
    end
end 
...
for i=1:6
   ...
   for i=1:540
      ...
      for j=2:121
          ...
          xcal(i,j)= finfft(trn(i,t),j-1);  %----loading fft train data  
...
% Loading fft test data
   ...
   for i=1:60
      ...
      for j=2:121
          ...
          tesxcal(i,j)= finfft(tes(i,t),j-1);      %----
...
S = ' The consolidated confusion matrix for ranked fft components ';
disp(S);
disp(confmat);
...
faccu = per/10;
S = ' The final accuracy for ranked fft components ';
disp(S);
disp(faccu);
